...
**No-play down exemption (2026-09-07, quellenübergreifend, nicht nur Hudl):** Der `downs_range`-Validierungscheck (`flag_football_ep.validation.checks.downs_range`, Details in `docs/pipeline.md` §4) verlangt normalerweise einen nicht-null `down`-Wert (0-4) für jede Zeile. Eine Ausnahme: eine Zeile mit `play_type == "no_play"` **und** `penalty == 1` (ein Foul ohne echten Spielzug, z. B. eine stehende Strafe zwischen zwei Downs) erreicht per Definition nie einen Snap und hat deshalb keinen eigenen Down-Wert — ein `null` dort ist keine Datenlücke, sondern die ehrliche Abbildung eines Play-Typs ohne Down-Konzept. Der Check bleibt für jede andere Zeile (jeder echte Spielzug, jede `no_play`-Zeile ohne `penalty == 1`) exakt so streng wie vorher; die Ausnahme ist eine Klassifikations-, keine Lockerungsentscheidung. Gefunden und behoben am Beispiel des IFAF-`/plays`-Feeds (38 von 46 zuvor blockierenden null-`down`-Zeilen über 17 Frauen-Spiele waren genau dieser Fall, davon einige erst nach einem zusätzlichen Fix: `penalty` muss bei einer vom Reviewer zurückgenommenen [`nullified`] Strafe-ohne-Spielzug-Zeile trotzdem gesetzt bleiben, sonst greift die Ausnahme nicht), gilt aber für jede Quelle, die `no_play`/`penalty` gemäß `canonical.PLAY_TYPE_VOCABULARY` setzt.
