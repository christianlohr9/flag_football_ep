...
**Accepted-game count in `plays.parquet` is unchanged at 15/29.** `score_reconstruction` passing is necessary but not sufficient for a game to leave quarantine — `partition_games` quarantines on *any* failing check, and 8 of the 9 newly-`score_reconstruction`-passing games are still quarantined for a pre-existing, unrelated `downs_range` finding (1–3 null `down` values per game, already present before this fix and out of this fix's scope). Fixing the running score therefore does not, by itself, move any of these 9 games into the accepted corpus that ultimately feeds `plays.parquet`/EP-WP training — that would require a separate fix to the `downs_range` gap. It does, however, fix what this Nachtrag set out to fix: the events-ledger's *own* per-game scoring reconstruction (used by `ingest_snapshots`'s per-game frame before quarantine, and by the standalone `ingest_snapshots(...)` call this doc's own exports script uses to build `ifaf_wm2026_pbp.csv`, tournaments=`None`/all games including quarantined ones) is now correct for these 9 games' entire run, not just their accepted rows.
