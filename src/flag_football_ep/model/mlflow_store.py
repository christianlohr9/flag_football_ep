...
def tracking_uri(config: Config) -> str:
    """The MLflow tracking URI: `MLFLOW_TRACKING_URI` if set, else the SQLite-backed URI for
    `config.paths.mlruns`.
...
def configure(config: Config) -> None:
    ...
    `MLFLOW_TRACKING_URI` is set: a remote store has no local `mlruns` directory to create.

    **A verified, load-bearing correction session-tested against the installed `mlflow`
    3.15.1:** `mlflow.set_tracking_uri()` itself writes `MLFLOW_TRACKING_URI` into
    `os.environ` as a side effect ("so that subprocess can inherit it",
    `mlflow/tracking/_tracking_service/utils.py`). Left unguarded, this creates a feedback
    loop with `tracking_uri()`'s own override check above: the FIRST local-sqlite `configure`
    call in a process would leak its resolved path into `os.environ[MLFLOW_TRACKING_URI]`,
    and every subsequent `tracking_uri()` call for a *different* `Config` (e.g. a different
    `tmp_path`-scoped store in a test suite) would then misread that leaked value as an
    intentional user override and get stuck on the first config's store forever. Guarded
    here: `os.environ`'s state is captured before calling `mlflow.set_tracking_uri`; if no
    override was actually present, the leaked value is popped back off immediately after,
    keeping this module's own override detection honest for the next call in the same
    process. When a real override *was* present (the normal `MLFLOW_TRACKING_URI=http://...`
    case this addendum exists for), nothing is touched -- the env var already holds the
    correct value.
    """
