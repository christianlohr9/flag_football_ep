...
- [ ] **Phase M3-2: EPA-Refinement** - Retrain EP/WP on the enlarged corpus (GroupKFold by game, calibration, per-tier eval); side-by-side with the HC's own "Scoring Probability by Situation" tables (SP/EP by down & distance, clustered/weighted) so he sees where the model agrees and where it improves; documented, reproducible, MLflow-versioned (Requirements: HC-03)
  ...
  - [ ] `M3-02-01-PLAN.md` — the two ingest fixes: unordered team-pair game segmentation, and the `half = 2` label sentinel scoped to games declared in `hc_games.csv` and never to `Copy of Data`, plus the decision table in `docs/data-contract.md` (HC-03)
  - [ ] `M3-02-02-PLAN.md` — training-side prerequisites: the EP `half`-feature sentinel that keeps head-coach rows distinguishable from real second-half plays, the label-construction guards for both EP and WP, and the three `competition_tier.csv` rows that stop the first fit dying on `UnmappedCompetitionError` (HC-03)
  - [ ] `M3-02-03-PLAN.md` — read-only snapshot of his nine aggregate tabs into `data/reference/hc_sp_tables/`, with the three Excel-corrupted distance-bin labels reconstructed and marked, plus its own PII and domain gate (HC-03)
...
| M3-1 HC-Workbook-Ingest | 3/4 | In progress (plan 01 at HC-question gate; real run done) | - |
| M3-2 EPA-Refinement | 0/8 | Planned | - |
| M3-3 Explosiveness & Efficiency | 0/3 | Planned | - |
