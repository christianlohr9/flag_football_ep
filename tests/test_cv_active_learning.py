...
def test_select_al_frames_iteration_2_excludes_iteration_1_selection(
    ...
    # target = 2 (half of `_MAX_CANDIDATES_PER_CLIP` = 4, plan AL-3's lowered cap):
    ...
    # before the cap was lowered from 12, then from 6) would consume every grid
    # candidate in iteration 1, leaving none for iteration 2's exclusion-disjoint proof.
    out_dir_1 = tmp_path / "al" / "iteration-1"
    selection_1 = select_al_frames(cfg, [session_id], 1, 2, 1, out_dir_1)

    out_dir_2 = tmp_path / "al" / "iteration-2"
    selection_2 = select_al_frames(cfg, [session_id], 2, 2, 1, out_dir_2)
