...
def test_checked_in_config_exclude_games(repo_root: Path) -> None:
    """2026-09-06 addendum: the IFAF ffwc26-men tournament is excluded from
    the corpus at ingest time (sources.ifaf.ingest_tournaments), not via a
    per-game training exclusion list -- exclude_games_ep/wp stay at just the
    original two legacy exclusions. See docs/ifaf-field-mapping.md."""
...
def test_checked_in_config_ifaf_ingest_tournaments_defaults_to_women_only(
    repo_root: Path,
) -> None:
    """2026-09-06 addendum: the corpus is safe by default -- ffwc26-men is
    snapshotted (fetch-ifaf --all-games) but never reaches plays.parquet
    unless a user explicitly adds it here."""
    cfg = load_config(repo_root / "ffep.toml")

    assert cfg.sources.ifaf.ingest_tournaments == ("ffwc26-women",)


def test_ifaf_ingest_tournaments_falls_back_to_tournament_when_unset(
    tmp_path: Path,
) -> None:
    """MINIMAL_TOML's `[sources.ifaf]` table has no `ingest_tournaments` key
    at all -- load_config must default it to a one-tuple of `tournament`."""
    config_path = tmp_path / "ffep.toml"
    config_path.write_text(MINIMAL_TOML, encoding="utf-8")
    cfg = load_config(config_path)
    assert cfg.sources.ifaf.ingest_tournaments == ("test-tournament",)


def test_ifaf_ingest_tournaments_explicit_list_overrides_default(
    tmp_path: Path,
) -> None:
    marker = 'tournament = "test-tournament"\napi_key_env = "CPX_API_KEY"'
    replacement = marker + '\ningest_tournaments = ["test-tournament", "test-tournament-2"]'
    config_path = tmp_path / "ffep.toml"
    config_path.write_text(MINIMAL_TOML.replace(marker, replacement), encoding="utf-8")
    cfg = load_config(config_path)
    assert cfg.sources.ifaf.ingest_tournaments == ("test-tournament", "test-tournament-2")
